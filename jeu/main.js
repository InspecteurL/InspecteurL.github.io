...
// Charger Idle, Walk et Attack séparément
async function loadCharacterAnimations() {
  const animNames = ["Idle", "Walk", "Attack"]; // noms des fichiers GLB
  for (const name of animNames) {
    await BABYLON.SceneLoader.ImportMeshAsync(
      ...
      const root = new BABYLON.TransformNode(`playerRoot_${name}`, scene);
      result.meshes.forEach(m => m.parent = root);
      root.scaling = new BABYLON.Vector3(0.01, 0.01, 0.01);
      root.position = new BABYLON.Vector3(0, 0, 0);

      if (!player.mesh) player.mesh = root; // racine principale
      if (result.skeletons[0]) player.skeleton = result.skeletons[0];

      result.animationGroups.forEach(anim => {
        animations[name.toLowerCase()] = anim;
      });
    });
  }
  playAnimation("idle");
